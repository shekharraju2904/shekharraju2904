...
import { LayoutDashboard, Wallet, Landmark, BarChart, UserCircle, Settings, LogOut, LogoIcon } from './Icons';
...
const SideNav: React.FC<SideNavProps> = ({ user, onLogout, activeView, setActiveView }) => {
            ...
            <aside className="hidden md:flex flex-col w-20 lg:w-64 h-screen p-4 bg-neutral-900/50 backdrop-blur-xl border-r border-white/10 text-white fixed z-40">
                ...
                <div className="border-t border-white/10 pt-4">
                    <div className="flex items-center gap-3 p-2">
                        ...
                        <button onClick={onLogout} className="p-2 ml-auto text-neutral-400 rounded-md hover:bg-white/10 hover:text-white hidden lg:block">
                            <LogOut size={20} />
                        </button>
                    </div>
                </div>
                ...
                <ul className="flex justify-around items-center h-full px-2">
                    {navItems.map(item => (
                     ...
                     <li>
                        <button
                            onClick={onLogout}
                            className="flex flex-col items-center justify-center p-2 rounded-lg w-16 text-neutral-400 hover:bg-white/10"
                        >
                            <LogOut size={24} />
